  ...
  <table :style="{marginTop:poolFixed?'86px':0}" v-if="!loading && stockList && stockList.length" class="stocklist-pool-table">
    ...
    <thead :class="{'fixed':poolFixed}" class="stocklist-pool-thead">
      <tr>
        ...
        <th style="width:290px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">戴帽原因</th>
        ...
        <!-- <sort-th style="width:100px;" label="戴帽日期" @sort="startSort" :sort-rule="sortRule" target-prop="special_treated_date" /> -->
        <th style="width:290px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">业绩预告</th>
        <!-- <sort-th style="width:80px;" :two-line="true" label="业绩预告<br/>时间" @sort="startSort" :sort-rule="sortRule" target-prop="next_performance_forecast_end_date" /> -->
        <sort-th style="width:120px;" :two-line="true" label="最新业绩报告<br/>归母净利润" @sort="startSort" :sort-rule="sortRule" target-prop="latest_net_profit" />
        <sort-th style="width:120px;" :two-line="true" label="最新每股<br/>净资产" @sort="startSort" :sort-rule="sortRule" target-prop="latest_net_asset_ps" />
        <!-- <sort-th style="width:80px;" :two-line="true" label="最新营业<br/>收入" @sort="startSort" :sort-rule="sortRule" target-prop="latest_operating_reenue" />
        <th style="width:80px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">所属板块</th> -->
    ...
    <tbody>
      <template v-for="(item,index) in stockList">
        <tr :ref="'tr_'+item.symbol" :key="item.symbol" class="stockitem-tbody-tr">
          ...
          <td :class="{'sort-target':isSortTarget('market_value',sortRule)}" class="td-center left">{{item.description}}</td>
          ...
          <!-- <td :class="{'sort-target':isSortTarget('special_treated_date',sortRule)}" class="td-center date">{{timeFormatter(item.special_treated_date,'YYYY/MM/DD')}}</td> -->
          <td class="td-center padding10 left">
            <span v-if="item.next_performance_forecast_end_date">{{timeFormatter(item.next_performance_forecast_end_date,'YYYY/MM/DD') + ":"+(item.next_performance_forecast_content)}}</span>
            <span v-else class="blank">--</span>
          </td>

          <!-- <td :class="{'sort-target':isSortTarget('next_performance_forecast_end_date',sortRule)}" class="td-center">
            <span>{{timeFormatter(item.next_performance_forecast_end_date,'YYYY/MM/DD')}}</span>
          </td> -->
          <td :class="{'sort-target':isSortTarget('latest_net_profit',sortRule)}" class="td-center">
            <span :class="colorFlag(item.latest_net_profit)">{{approxNumber(item.latest_net_profit)}}</span><span class="sub-date">{{timeFormatter(item.latest_report_end_date,'YYYY/MM/DD')}}</span></td>
          <td :class="{'sort-target':isSortTarget('latest_net_asset_ps',sortRule)}" class="td-center">
            <span :class="colorFlag(item.latest_net_asset_ps)">{{item.latest_net_asset_ps}}</span></td>
          <!-- <td :class="{'sort-target':isSortTarget('latest_operating_reenue',sortRule)}" class="td-center">
            <span :class="color(item.latest_operating_reenue)">{{approxNumber(item.latest_operating_reenue)}}</span></td>
          <td  class="td-center plates">
            <template v-if="plateName(item.plates) && plateName(item.plates).length"><a :href="`/theme/${p.plate_id}`" target="_blank" :key="p.plate_id"  v-for="p in plateName(item.plates)">{{p.plate_name}}</a></template><span v-else class="blank">暂无所属板块</span>
          </td> -->
          <!-- <td class="td-center">{{plateName(item.plates)}}</td> -->
...
export default {
  ...
  methods: {
    ...
    plateName (data) {
      return share
        .dataBeautify(data, "fields", "items")
    ...
    plateName (data) {
      return share.dataBeautify(data, "fields", "items");
    },
