  ...
  <table :style="{marginTop:poolFixed?'86px':0}" v-if="!loading && stockList && stockList.length" class="stocklist-pool-table">
    ...
    <thead :class="{'fixed':poolFixed}" class="stocklist-pool-thead">
      <tr>
        ...
        <sort-th style="width:100px;" label="最新价" @sort="startSort" :sort-rule="sortRule" target-prop="cur_price" />
        <sort-th style="width:100px;" label="涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="px_change_rate" />
        <!-- <sort-th style="width:100px;" label="送转派息" @sort="startSort" :sort-rule="sortRule" target-prop="bonus_share_ratio" /> -->
        ...
        <sort-th style="width:100px;" :two-line="true" label="预案公告后<br/>最大涨幅" @sort="startSort" :sort-rule="sortRule" target-prop="max_pcp_after_advance" />
        <sort-th style="width:100px;" :two-line="true" label="预案公告后<br/>至今涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="pcp_after_advance" />
         <sort-th style="width:100px;" :two-line="false" label="流通市值" @sort="startSort" :sort-rule="sortRule" target-prop="circulation_value" />
         <sort-th style="width:100px;" :two-line="false" label="除权后股价" @sort="startSort" :sort-rule="sortRule" target-prop="px_after_divi" />
         <!-- <sort-th style="width:90px;" :two-line="false" label="股东是否减持" @sort="startSort" :sort-rule="sortRule" target-prop="major_holder_reduction" /> -->
         <sort-th style="width:100px;" :two-line="false" label="上市日期" @sort="startSort" :sort-rule="sortRule" target-prop="listed_date" />
      </tr>
    </thead>
    ...
    <tbody>
      <template  v-for="(item,index) in stockList">
        <tr :ref="'tr_'+item.symbol" :key="item.symbol" class="stockitem-tbody-tr">
          ...
          <td :class="{'sort-target':isSortTarget('bonus_share_ratio',sortRule)}" class="td-center">
            <span v-if="item.bonus_share_ratio ||item.tran_add_share_ratio">10<span v-if="item.bonus_share_ratio">送{{item.bonus_share_ratio}}</span><span v-if="item.tran_add_share_ratio">转增{{item.tran_add_share_ratio}}</span><span v-if="item.cash_divi_RMB">派{{item.cash_divi_RMB}}</span></span></td>
          <td :class="{'sort-target':isSortTarget('ex_divi_date',sortRule)}" class="td-center padding10"><span>{{timeFormatter(item.ex_divi_date,'YYYY/MM/DD')}}</span></td>
          ...
          <td :class="{'sort-target':isSortTarget('px_after_divito',sortRule)}" class="td-center">{{(item.px_after_divi)}}</td>
