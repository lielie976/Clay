  ...
  <table :style="{marginTop:poolFixed?'86px':0}" v-if="!loading && stockList && stockList.length" class="stocklist-pool-table">
    ...
    <thead :class="{'fixed':poolFixed}" class="stocklist-pool-thead">
      <tr>
        ...
        <th style="width:260px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">理由分析</th>
        <sort-th style="width:80px;" label="最新价" @sort="startSort" :sort-rule="sortRule" target-prop="cur_price" />
        <sort-th style="width:80px;" label="涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="px_change_rate" />
        <sort-th style="width:80px;" label="封单比" @sort="startSort" :sort-rule="sortRule" target-prop="blockade_ratio" />
        <sort-th style="width:80px;" label="换手率" @sort="startSort" :sort-rule="sortRule" target-prop="turnover_ratio" />
        <sort-th style="width:80px;" label="流通市值" @sort="startSort" :sort-rule="sortRule" target-prop="circulation_value" />
        <sort-th style="width:80px;" label="总市值" @sort="startSort" :sort-rule="sortRule" target-prop="total_value" />
        <sort-th style="width:80px;" label="首次封板" @sort="startSort" :sort-rule="sortRule" target-prop="first_raise_time" />
        <sort-th style="width:80px;" label="最后封板" @sort="startSort" :sort-rule="sortRule" target-prop="last_raise_time" />
        <sort-th style="width:80px;" label="开板数" @sort="startSort" :sort-rule="sortRule" target-prop="open_count" />
        <sort-th style="width:80px;" label="连板数" @sort="startSort" :sort-rule="sortRule" target-prop="continue_board_count" />
      </tr>
    </thead>
    ...
    <tbody>
      <template  v-for="(item,index) in stockList">
        <tr :ref="'tr_'+item.symbol" :key="item.symbol" class="stockitem-tbody-tr">
          ...
          <td :class="{'sort-target':isSortTarget('blockade_ratio',sortRule)}" class="td-center">
            {{changeRate(item.blockade_ratio)}}</td>
          <td :class="{'sort-target':isSortTarget('turnover_ratio',sortRule)}" class="td-center">{{changeRate(item.turnover_ratio)}}</td>
          <td :class="{'sort-target':isSortTarget('circulation_value',sortRule)}" class="td-center"><span >{{approxNumber(item.circulation_value)}}</span></td>
 <td :class="{'sort-target':isSortTarget('total_value',sortRule)}" class="td-center"><span >{{approxNumber(item.total_value)}}</span></td>
          <td :class="{'sort-target':isSortTarget('first_raise_time',sortRule)}" class="td-center"><span >{{timeFormatter(item.first_raise_time,'YYYY/MM/DD')}}</span></td>
          <td :class="{'sort-target':isSortTarget('last_raise_time',sortRule)}" class="td-center"><span >{{timeFormatter(item.last_raise_time,'YYYY/MM/DD')}}</span></td>
          <td :class="{'sort-target':isSortTarget('open_count',sortRule)}" class="td-center">{{item.open_count}}</td>
          <td :class="{'sort-target':isSortTarget('continue_board_count',sortRule)}" class="td-center">{{item.continue_board_count}}</td>
...
export default {
  ...
  methods: {
    ...
    plateName(data){
      return share.dataBeautify(data, 'fields', 'items').map(item=>item.plate_name).join(' ')
    },
    getRateColor(val) {
      if (val > 0) {
        return '#D93641';
