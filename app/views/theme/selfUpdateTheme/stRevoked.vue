  ...
  <table :style="{marginTop:poolFixed?'86px':0}" v-if="!loading && stockList && stockList.length" class="stocklist-pool-table">
    ...
    <thead :class="{'fixed':poolFixed}" class="stocklist-pool-thead">
      <tr>
        ...
        <th style="width:230px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">摘帽原因</th>
        <sort-th style="width:150px;" label="最新价" @sort="startSort" :sort-rule="sortRule" target-prop="cur_price" />
        <sort-th style="width:150px;" label="涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="px_change_rate" />
        <sort-th style="width:150px;" label="摘帽日期" @sort="startSort" :sort-rule="sortRule" target-prop="revoked_date" />
        <sort-th style="width:150px;" label="摘帽至今涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="pcp_since_revoked" />
         <th style="width:230px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">所属板块</th>
    ...
    <tbody>
      <template  v-for="(item,index) in stockList">
        <tr :ref="'tr_'+item.symbol" :key="item.symbol" class="stockitem-tbody-tr">
          ...
          <td :class="{'sort-target':isSortTarget('market_value',sortRule)}" class="td-center left">{{item.revoked_reason}}</td>
          ...
          <td :class="{'sort-target':isSortTarget('revoked_date',sortRule)}" class="td-center">{{timeFormatter(item.revoked_date,'YYYY/MM/DD')}}</td>
         <td class="td-center" :class="{'sort-target':isSortTarget('pcp_since_revoked',sortRule)}">
            <span :class="color(item.pcp_since_revoked)">{{changeRate(item.pcp_since_revoked)}}</span>
          </td>
