  ...
  <table :style="{marginTop:poolFixed?'86px':0}" v-if="!loading && stockList && stockList.length" class="stocklist-pool-table">
    ...
    <thead :class="{'fixed':poolFixed}" class="stocklist-pool-thead">
      <tr>
        ...
        <th style="width:300px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">停牌原因</th>
        ...
        <sort-th style="width:120px;" label="复牌日期" @sort="startSort" :sort-rule="sortRule" target-prop="resumption_date" />
        <sort-th style="width:120px;" label="复牌至今涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="pcp_since_resume" />
        <sort-th style="width:160px;" :two-line="false" label="停牌期间指数涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="index_pcp_when_suspend" />
        <sort-th style="width:120px;" :two-line="false" label="停牌时长" @sort="startSort" :sort-rule="sortRule" target-prop="suspend_day_count" />
      </tr>
    </thead>
    ...
    <tbody>
      <template  v-for="(item,index) in stockList">
        <tr :ref="'tr_'+item.symbol" :key="item.symbol" class="stockitem-tbody-tr">
          ...
          <td :class="{'sort-target':isSortTarget('market_value',sortRule)}" class="td-center left">
            {{item.resumed_reason}}
          </td>
          ...
          <td :class="{'sort-target':isSortTarget('resumption_date',sortRule)}" class="td-center">{{timeFormatter(item.resumption_date,'YYYY/MM/DD')}}</td>
          <td :class="{'sort-target':isSortTarget('pcp_since_resume',sortRule)}" class="td-center"><span :class="color(item.pcp_since_resume)">{{changeRate(item.pcp_since_resume)}}</span></td>

          <td :class="{'sort-target':isSortTarget('index_pcp_when_suspend',sortRule)}" class="td-center"><span :class="color(item.index_pcp_when_suspend)">{{changeRate(item.index_pcp_when_suspend)}}</span></td>
          <td :class="{'sort-target':isSortTarget('suspend_day_count',sortRule)}" class="td-center">{{item.suspend_day_count}}天</td>
