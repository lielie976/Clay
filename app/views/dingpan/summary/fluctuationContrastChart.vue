...
<div class="hit-pool-container">
   <div id="fluctuation" class="hit-pool-chart" style="width: 180px;height:96px;" ref="chart">

  </div>
  <div class="hit-pool-title" id="fluctuation-title">涨跌对比<span id="fluctuation-title-number"><span
  class="-market-color--red">{{raiseList.length?raiseList[raiseList.length - 1]:0}}</span>：<span class="-market-color--green">{{downList.length?downList[downList.length - 1]:0}}</span></span></div>
  <div id="fluctuation-c-label"><p>{{maxValue}}</p><p>{{maxValue-midValue}}</p><p>{{minValue}}</p></div>
</div>
...
export default {
  ...
  watch: {
    timestampList() {
      ...
      this.minValue =
        this.raiseList && this.raiseList.length && this.downList && this.downList.length
          ? Math.min.apply(null, this.raiseList.concat(this.downList))
          : 0;
  ...
  methods: {
    draw: function() {
      ...
      //      if (!$('#fluctuation-c-label').length) {
      //   $('#fluctuation').append(`<div id="fluctuation-c-label"><p>${this.maxValue}</p><p>${this.maxValue-this.midValue}</p><p>${this.minValue}</p></div>`)
      // } else {
      //   $('#fluctuation-c-label').replaceWith(`<div id="fluctuation-c-label"><p>${this.maxValue}</p><p>${this.maxValue-this.midValue}</p><p>${this.minValue}</p></div>`)
      // }
