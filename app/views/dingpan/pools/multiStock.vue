  ...
  <table v-if ="topFixed" class="table hit-pool__table fixed">
    ...
    <thead class="hit-pool__table-head">
      <tr>
        ...
        <th @click="startSort('turnover_ratio')" target="turnover_ratio" :class="{'sort_target':'turnover_ratio' == sortTarget}" class="stock-sort"><abbr title="换手率">换手率</abbr><span class="icon"><i :class="getSortStatus('turnover_ratio')" class="iconfont " ></i></span></th>
        <th  @click="startSort('circulation_value')" target="circulation_value" :class="{'sort_target':'circulation_value' == sortTarget}" class="stock-sort"><abbr title="流通市值">流通市值</abbr><span class="icon"><i :class="getSortStatus('circulation_value')" class="iconfont " ></i></span></th>
        ...
        <th :class="{'sort_target':'continue_board_count' == sortTarget}" @click="startSort('continue_board_count')" style="width: 88px;min-width:88px;"   target="continue_board_count" class="stock-sort"><abbr title="连板数">连板数</abbr><span class="icon"><i :class="getSortStatus('continue_board_count')" class="iconfont " ></i></span></th>
        <th   @click="startSort('board_days')" :class="{'sort_target':'board_days' == sortTarget}" target="board_days" class="stock-sort stock-sort__big"><abbr title="几天几板">几天几板</abbr><span class="icon"><i :class="getSortStatus('board_days')"  class="iconfont" ></i></span></th>
      </tr>
  ...
  <table class="table hit-pool__table">
    ...
    <tbody class="table hit-pool__table-body">
      <tr :key="item.symbol" v-for="item in stockList">
          ...
          <td  :class="{'sort_target':'board_days' == sortTarget}" target="board_days" >{{item.board_days}}天{{item.board_count}}板</td>
        </tr>
    </tbody>
  </table>
