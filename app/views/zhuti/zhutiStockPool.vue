      ...
      <table :style="{marginTop:poolFixed?'86px':0}" v-if="filterList && filterList.length && !isSpecial" class="stocklist-pool-table">
        ...
        <thead :class="{'fixed':poolFixed}" class="stocklist-pool-thead">
          <tr>
            ...
            <sort-thead style="width:90px;" label="最新价" @sort="startSort" :sort-rule="sortRule" target-prop="last_px" />
            <sort-thead style="width:90px;" label="涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="px_change_rate" />
            <sort-thead style="width:90px;" label="换手率" @sort="startSort" :sort-rule="sortRule" target-prop="turnover_ratio" />
            <sort-thead style="width:90px;"  :two-line="true" label="自由流<br/>通市值" @sort="startSort" :sort-rule="sortRule" target-prop="circulation_value" />
            <sort-thead style="width:90px;" label="总市值" @sort="startSort" :sort-rule="sortRule" target-prop="market_value" />
            <sort-thead style="width:90px;" label="市盈率" @sort="startSort" :sort-rule="sortRule" target-prop="pe_rate" />
            <sort-thead style="width:90px;" label="资金流向" @sort="startSort" :sort-rule="sortRule" target-prop="flow" />
            <th style="width:70px;" class="stocklist-pool-th" >叠加</th>
          </tr>
        </thead>
