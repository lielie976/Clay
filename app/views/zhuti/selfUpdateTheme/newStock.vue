<template>
  <table :style="{marginTop:poolFixed?'86px':0}" v-if="!loading && stockList && stockList.length" class="stocklist-pool-table">
    ...
    <thead :class="{'fixed':poolFixed}" class="stocklist-pool-thead">
      <tr>
        ...
        <sort-th style="width:120px;" label="总市值" @sort="startSort" :sort-rule="sortRule" target-prop="total_value" />
        <sort-th style="width:120px;" label="首次开板" @sort="startSort" :sort-rule="sortRule" target-prop="first_open_time" />
        <sort-th style="width:120px;" label="开板前连板数" @sort="startSort" :sort-rule="sortRule" target-prop="before_open_count" />
        <sort-th style="width:120px;" :two-line="false" label="累计涨幅" @sort="startSort" :sort-rule="sortRule" target-prop="accumulated_pcp" />
        <sort-th style="width:100px;" :two-line="false" label="上市日期" @sort="startSort" :sort-rule="sortRule" target-prop="time_on_market" />
        <!-- <th style="width:100px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">所属行业</th> -->
      </tr>
    </thead>
    ...
    <tbody>
      <template  v-for="(item,index) in stockList">
        <tr :ref="'tr_'+item.symbol" :key="item.symbol" class="stockitem-tbody-tr">
          ...
          <!-- <td :class="{'sort-target':isSortTarget('market_value',sortRule)}" class="td-center"></td> -->
 ...
 <td :class="{'sort-target':isSortTarget('total_value',sortRule)}" class="td-center"><span >{{approxNumber(item.total_value)}}</span></td>
          <td :class="{'sort-target':isSortTarget('first_open_time',sortRule)}" class="td-center"><span >{{timeFormatter(item.first_open_time,'YYYY/MM/DD')}}</span></td>
          <td :class="{'sort-target':isSortTarget('suspend_day_count',sortRule)}" class="td-center">{{item.before_open_count}}</td>
          <td :class="{'sort-target':isSortTarget('accumulated_pcp',sortRule)}" class="td-center"><span :class="color(item.accumulated_pcp)">{{changeRate(item.accumulated_pcp)}}</span></td>
