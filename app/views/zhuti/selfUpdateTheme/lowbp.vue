  ...
  <table :style="{marginTop:poolFixed?'86px':0}" v-if="!loading && stockList && stockList.length" class="stocklist-pool-table">
    ...
    <thead :class="{'fixed':poolFixed}" class="stocklist-pool-thead">
      <tr>
        ...
        <!-- <th style="width:300px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">摘帽原因</th> -->
        <sort-th style="width:130px;" label="最新价" @sort="startSort" :sort-rule="sortRule" target-prop="cur_price" />
        <sort-th style="width:130px;" label="涨跌幅" @sort="startSort" :sort-rule="sortRule" target-prop="px_change_rate" />
        <sort-th style="width:130px;" label="市净率" @sort="startSort" :sort-rule="sortRule" target-prop="PB" />
        <sort-th style="width:130px;" label="市盈率" @sort="startSort" :sort-rule="sortRule" target-prop="PE" />
         <sort-th style="width:130px;" label="总市值" @sort="startSort" :sort-rule="sortRule" target-prop="total_value" />
         <th style="width:280px;" class="stocklist-pool-th stocklist-pool-th-name-threeline">所属板块</th>
         <sort-th style="width:130px;" label="净资产收益率" @sort="startSort" :sort-rule="sortRule" target-prop="WROE" />
      </tr>
    </thead>
    ...
    <tbody>
      <template  v-for="(item,index) in stockList">
        <tr :ref="'tr_'+item.symbol" :key="item.symbol" class="stockitem-tbody-tr">
          ...
          <!-- <td :class="{'sort-target':isSortTarget('market_value',sortRule)}" class="td-center">{{item.revoked_reason}}</td> -->
          ...
          <td class="td-center" :class="{'sort-target':isSortTarget('px_change_rate',sortRule)}">
           ...
           <td class="td-center" :class="{'sort-target':isSortTarget('PB',sortRule)}">
            <span >{{changePositive(item.PB)}}</span>
          </td>
          <td class="td-center" :class="{'sort-target':isSortTarget('PE',sortRule)}">
            <span >{{changePositive(item.PE)}}</span>
          </td>
            <td class="td-center" :class="{'sort-target':isSortTarget('TTM',sortRule)}">
            <span >{{approxNumber(item.total_value)}}</span>
          </td>

          <td class="td-center plates"><template v-if="plateName(item.plates) && plateName(item.plates).length"><a :href="`/theme/${p.plate_id}`" :key="p.plate_id" target="_blank" v-for="p in plateName(item.plates)">{{p.plate_name}}</a></template><span v-else class="blank">暂无所属板块</span></td>
          <td class="td-center" :class="{'sort-target':isSortTarget('WROE',sortRule)}">
            <span >{{changeNegative(item.WROE)}}</span>
          </td>
