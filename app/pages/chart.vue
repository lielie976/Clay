...
export default {
  async asyncData ({ store }) {
    try {
      let fenshiData = ['000300.SS', '000777.SZ', '300104.SZ', '300168.SZ', '002142.SZ']
      await store.dispatch('stockTrend/getTrend', fenshiData.join(','))
      return {
        chooseFenshi: fenshiData
      }
    } catch (err) {
      console.log(err)
    }
  ...
  computed: {
    ...
    preClose () {
      return this.$store.state.stockTrend.preValue
    }
  },
  mounted () {
    this.init()
    ...
    init () {
      ...
      let pattern = {
        data_source: {
          data: this.trendRate,
          fields: fields,
          time_ranges: [[this.selectedDate.setHours(9, 30, 0, 0), new Date().setHours(11, 30, 0, 0)], [this.selectedDate.setHours(13, 1, 0, 0), new Date().setHours(15, 0, 0, 0)]]
        },
        price_precision: 2,
        style: {
          font: {family: 'Microsoft YaHei', size: 14},
          padding: {top: 1, right: 100, bottom: 28, left: 1},
          wheel_zoom_step: 1,
      ...
      [this.canvas_el, this.ia_canvas_el, this.mid_canvas_el] = this.initCanvas(container, pattern)
      this.genCtx()
